...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("pdf_path", nargs='?', type=str, help="Path to the PDF file (optional)")
    ...
    def handle(self, *args, **options):
        ...
        # If no pdf_path provided, default to the curated source PDF in this package
        if not pdf_path:
            default_pdf = os.path.join(os.path.dirname(__file__), "sources", "baybayin_ancient_script.pdf")
            if os.path.exists(default_pdf):
                pdf_path = default_pdf
                self.stdout.write(f"No PDF path provided — using default: {pdf_path}")
            else:
                self.stderr.write("No PDF path provided and default PDF not found in sources folder.")
                return
